...
//    IntEnable(INT_TIMER0A);
    ...
    //Boost s1=PH1, s2=PK6, sel=JP4=PD4?
...
void button_debounce(uint32_t buttons)
{
	int32_t i, mask;
...
void button_read_joystick(void)
{
...
uint32_t button_auto_repeat(void)
{
...
void button_task_func(void) {
    while (true) {
        ...
        uint32_t gpio_buttons =
            ...
            ((~GPIOPinRead(GPIO_PORTH_BASE, 0xff) & GPIO_PIN_1) << 1) | //S1 = PH1 = Bit2
            ((~GPIOPinRead(GPIO_PORTK_BASE, 0xff) & GPIO_PIN_6) >> 3) | //S2 = PK6 = Bit3
            ((~GPIOPinRead(GPIO_PORTD_BASE, 0xff) & GPIO_PIN_4));  //Select = JP4=PD4 = Bit4

        uint32_t old_buttons = gButtons;    // save previous button state
        button_debounce(gpio_buttons);       // Run the button debouncer. The result is in gButtons.
        button_read_joystick();               // Convert joystick state to button presses. The result is in gButtons.
        uint32_t presses = ~old_buttons & gButtons;   // detect button presses (transitions from not pressed to pressed)
        presses |= button_auto_repeat();      // autorepeat presses if a button is held long enough
        ...
        // Push button presses into the FIFO
        int i;
        for (i = 0; i <= 8; ++i) { // There are 9 buttons in total
            if (presses & (1 << i)) { // Check each button press
                Button btn = (Button)i;
                Mailbox_post(buttons_mail, &btn, BIOS_NO_WAIT);
//                ButtonFIFOPut((Button)i);
            }
        }
    }
}

void button_clock_func(void) {
